...
import at.scch.mbteclo.state.Configuration;
...
public class EditBugPage extends AbstractLoggedinBugzillaPage {
	...
	public String getVersion() {
		if(ConfigurationOption.AddVersion.isActive()) {
			...
			WebElement bugVersion = driver.findElement(By.xpath("//tr[@id='field_tablerow_version']/td"));
	...
	public AbstractBugzillaPage editState(String status) {
		...
		} else if(ConfigurationOption.CommentOnChangeResolution.isActive()) {
	...
	public AbstractBugzillaPage markAsDuplicate(int originalId){
		clickMarkAsDuplicate();
		setBugDuplicateOf(originalId);
		commitButton.click();
		if(ConfigurationOption.CommentOnAllTransitions.isActive() || ConfigurationOption.CommentOnChangeResolution.isActive() || ConfigurationOption.CommentOnDuplicate.isActive()){
			return PageFactory.initElements(driver, ErrorCommentRequiredPage.class);
		}
		return PageFactory.initElements(driver, BugChangedPage.class);
	}
