...
public class BugLifecycleModel extends AbstractModel {
	...
	public boolean toConfirmedGuard() {
		...
		if(state.getPage() != BugzillaState.Page.EditBug){
			return false;
		}
	...
	public void toResolvedOpenBlockers() {
		...
		state.setPage(BugzillaState.Page.ErrorOpenBlockers);
		state.setScenario(null);
		state.setCurrentlyOpenBug(null);
	}
	...
	public boolean toResolvedOpenBlockersGuard() {
		...
		String status = bug.getStatus();
		return state.getPage() == BugzillaState.Page.EditBug && ("UNCONFIRMED".equals(status) || "CONFIRMED".equals(status) || "IN_PROGRESS".equals(status));
	}
	/*#end*/
	...
	public boolean resolutionGuard() {
		...
		String status = bug.getStatus();
		return ("RESOLVED".equals(status) || "VERIFIED".equals(status)) && state.getPage() == BugzillaState.Page.EditBug;
	}
...
//	/*#end*/
	...
	public void markAsDuplicate() {
		...
		if(circle){
			...
			state.setPage(BugzillaState.Page.ErrorCircularDuplicates);
		} else {
	...
	public boolean markAsDuplicateGuard() {
		...
		return ("UNCONFIRMED".equals(status) || "CONFIRMED".equals(status) || "IN_PROGRESS".equals(status) || "RESOLVED".equals(status)) &&
			state.getPage() == BugzillaState.Page.EditBug && state.getBugs().size() >= 2 && !"DUPLICATE".equals(resolution);
	}

	/*#if ($CommentOnAllTransitions)*/
	@Guard("toConfirmedWithoutComment")
	public boolean guardWithoutComment() {
		return state.getPage() == BugzillaState.Page.EditBug;
	}

	@TestStep("toConfirmedWithoutComment")
	public void toConfirmedWithoutComment() {
		adapter.editState("CONFIRMED");
		state.setPage(BugzillaState.Page.ErrorCommentRequired);
		state.setScenario(null);
		state.setCurrentlyOpenBug(null);
	}
	...
	public boolean toUnconfirmedGuardWithoutComment() {
		...
		String status = bug.getStatus();
		return state.getPage() == BugzillaState.Page.EditBug && ("RESOLVED".equals(status) || "VERIFIED".equals(status));
	}
	/*#end*/
	...
	public boolean toInProgressWithoutCommentGuard() {
		...
		String status = bug.getStatus();
		return state.getPage() == BugzillaState.Page.EditBug && ("UNCONFIRMED".equals(status) || "CONFIRMED".equals(status));
	}
	/*#end*/
	/*#end*/
	...
	public boolean toVerifiedWithoutCommentGuard() {
		...
		String status = bug.getStatus();
		return state.getPage() == BugzillaState.Page.EditBug && "RESOLVED".equals(status);
	}
	/*#end*/
	/*#end*/
	...
	public boolean markAsDuplicateWithoutCommentGuard() {
		...
		/*#end*/
		return guardResult && state.getPage() == BugzillaState.Page.EditBug && state.getBugs().size() >= 2 && !"DUPLICATE".equals(resolution);
	}
	/*#end*/
	/*#end*/
}
